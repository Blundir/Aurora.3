...
/datum/event/rescue_pod/announce()
	if(prob(66))
		command_announcement.Announce("Ship sensors indicate an escape pod inbound to a nearby celestial body. Investigate the situation if necessary and hand off the survivors, if any, to a third party vessel.", new_title="CSCSS Falconhurst Sensor Suite", new_sound = 'sound/AI/commandreport.ogg', zlevels = affecting_z)
